...
from help import send_message, sleeping_between_wallets, intro, outro, send_list, convert_to
from settings import bot_status, bot_id, bot_token, shuffle, withdraw_from_okex
from modules.myaccount import Account
from modules.okex import Okex
from modules.xyfinance import xyFinance

#------orbiter-options------#
from_chain = 'Linea'                                             # не менять
...
def main():
    ...
    for idx, (wallet, proxy) in enumerate(data, start=1):
        ...
        try:
            ...
            send_list.append(xyFinance(account.id, account.private_key, account.proxy, from_chain).main())

            if addressokx != None:
                send_list.append(Okex(account.id, account.private_key, account.proxy, from_chain).deposit_to_okex(addressokx))
